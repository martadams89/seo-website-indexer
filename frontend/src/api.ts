...
export interface EntityDiscovery { source_url: string; discovered_at: string; schema_types: string[]; sources: string[]; found_fields: string[]; warnings: string[]; selection: { selected_name: string; selected_type: string; reason: string; candidates: Array<{ name: string; type: string; url: string; selected: boolean; relationship?: string }> }; data: { name: string; market: string; locale: string; entity_type: string; primary_url: string; address: string; phone: string; identifiers: Record<string,string>; listings: LocalEntity['listings']; knowledge: Record<string,unknown>; review_rating: number | null; review_count: number | null } }
