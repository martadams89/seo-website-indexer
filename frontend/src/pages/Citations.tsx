...
export default function CitationsPage() {
  ...
  return <div className="ops-page citations-page">
    ...
    {watchlistOpen && <Modal onClose={() => setWatchlistOpen(false)} size="sm" className="prompt-watchlist-modal" eyebrow="Competitive context" title="Source watchlist" description="When these domains appear in citations, the source graph will label them as competitors." icon={<Target/>} footer={<><button className="btn btn-ghost" onClick={() => setWatchlistOpen(false)}>Cancel</button><button className="btn btn-primary" disabled={savingConfig} onClick={saveCompetitors}>{savingConfig ? <Loader2 size={13} className="spin"/> : <Save size={13}/>} Save watchlist</button></>}><label>Competitor domains<textarea data-autofocus rows={7} value={competitorDomains} onChange={event => setCompetitorDomains(event.target.value)} placeholder={'competitor.com\nanother-rival.co.uk'}/><small>Separate domains with spaces, commas or new lines. Do not include paths.</small></label></Modal>}
