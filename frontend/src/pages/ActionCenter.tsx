...
export default function ActionCenterPage() {
  ...
  return <div className="ops-page action-centre-page">
    ...
    {composer&&<div className="ops-modal-backdrop" onMouseDown={()=>setComposer(false)}><div className="ops-modal" onMouseDown={e=>e.stopPropagation()}><header><div><span className="eyebrow">Create accountable work</span><h2>New action</h2></div><button className="btn-icon btn-icon-ghost" onClick={()=>setComposer(false)}><X/></button></header><div className="form-grid"><label className="full">Title<input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})} autoFocus placeholder="What needs to change?"/></label><label className="full">Description<textarea value={draft.description} onChange={e=>setDraft({...draft,description:e.target.value})} rows={4} placeholder="Why it matters and what evidence supports it"/></label><label>Severity<select value={draft.severity} onChange={e=>setDraft({...draft,severity:e.target.value})}>{SEVERITIES.map(value=><option key={value}>{value}</option>)}</select></label><label>Owner<select value={draft.assignee_user_id} onChange={e=>setDraft({...draft,assignee_user_id:e.target.value})}><option value="">Unassigned</option>{members.map(member=><option key={member.user_id} value={member.user_id}>{member.name||member.email}</option>)}</select></label><label>Due date<input type="date" value={draft.due_at} onChange={e=>setDraft({...draft,due_at:e.target.value})}/></label></div><footer><button className="btn btn-ghost" onClick={()=>setComposer(false)}>Cancel</button><button className="btn btn-primary" disabled={!canManage||busy==='create'||!draft.title.trim()} onClick={create}>{busy==='create'?'Creating…':'Create action'}</button></footer></div></div>}
  </div>;
}
