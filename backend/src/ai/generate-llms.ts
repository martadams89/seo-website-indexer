...
import { complete, generationProvider, GEN_KEY, type GenProvider } from './complete.js';
import { readResponseText, safeFetch } from '../security/outbound-url.js';

// ── Provider text completion (no web search) ─────────────────────────────────
// The provider dispatch lives in ./complete.ts so the Ranking Playbook can
// draft with the same keys and model choices.

export type { GenProvider } from './complete.js';

export function llmsGenerationProvider(workspaceId: string | null = null): GenProvider | null {
  return generationProvider(workspaceId);
}
